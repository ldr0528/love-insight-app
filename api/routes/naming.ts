...
router.post('/generate', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (type === 'baby') {
      const { lastName, gender, birthYear, birthMonth, birthDay, birthTime, styles, meanings, nameLength, avoidChars, fixedChar } = params
      const styleStr = styles?.length ? styles.join('、') : '不限'
      const meaningStr = meanings?.length ? meanings.join('、') : '不限'
      
      prompt = `请为姓氏“${lastName || '李'}”的${gender === 'boy' ? '男孩' : '女孩'}起3个名字。
